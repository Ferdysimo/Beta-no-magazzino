...
SERVER_GIT_COMMIT = _git_commit_short()
...
@api_router.post("/diagnostics/frontend")
...
@api_router.post("/diagnostics/frontend/error")
...
def _data_integrity_issue(
    ...
    *,
    ...
    parts = [kind, restaurant_id or "global", date or "none", str(expected or ""), str(actual or "")]
    issues.append({
        "id": ":".join(parts)[:240],
        "severity": severity,
        "area": area,
...
def _integrity_issue_sort_key(issue: Dict):
    rank = {"critical": 0, "warning": 1, "info": 2}
    return (
        rank.get(issue.get("severity"), 9),
        str(issue.get("date") or "9999-99-99"),
        str(issue.get("restaurant_label") or ""),
        str(issue.get("kind") or ""),
    )


@api_router.get("/admin/data-integrity")
async def get_admin_data_integrity(
    days: int = 60,
    token_data: dict = Depends(verify_token),
):
    if token_data.get("role") != "admin":
    ...
    cutoff = (datetime.now(ROME_TZ).date() - timedelta(days=days - 1)).isoformat()
    ...
    restaurants = await db.restaurants.find(
        {"role": "restaurant"},
        {"_id": 0, "id": 1, "username": 1, "location": 1, "order_counter": 1},
    ).sort("location", 1).to_list(500)
    rest_map = {r.get("id"): (r.get("location") or r.get("username") or r.get("id") or "") for r in restaurants}

    valid_beverage_siglas = {b["sigla"] for b in BEVERAGES_CATALOG}
    for rest in restaurants:
        rid = rest.get("id")
        if not rid:
            continue
        label = rest_map.get(rid, rid)
        audit_docs = await db.cash_audit_log.find(
            {"restaurant_id": rid, "date_rome": {"$gte": cutoff, "$lte": today}},
            {"_id": 0, "date_rome": 1, "category": 1, "field": 1},
        ).to_list(5000)
        manual_audits = {
            (d.get("date_rome"), d.get("category"), d.get("field"))
            for d in audit_docs
        }
        ...
        cash_docs = await db.cash_daily_counts.find(
            {"restaurant_id": rid, "date_rome": {"$gte": cutoff, "$lte": today}},
            {"_id": 0},
        ).sort("date_rome", 1).to_list(days + 10)
        dmap = await _get_pasta_dict_for(rid)
        for cash_doc in cash_docs:
            date_str = cash_doc.get("date_rome") or ""
            if not date_str:
                continue
            prev_cash = await db.cash_daily_counts.find_one(
                {"restaurant_id": rid, "date_rome": {"$lt": date_str}},
                sort=[("date_rome", -1)],
                projection={"_id": 0},
            )
            if prev_cash:
                prev_date = prev_cash.get("date_rome") or ""
                prev_bev_docs = await db.beverage_daily_counts.find(
                    {"restaurant_id": rid, "date_rome": prev_date},
                    {"_id": 0},
                ).to_list(100)

                checks["cash_mattina"] += 1
                expected_mattina = _format_report_number(round(_compute_cash_sera_full(prev_cash, prev_bev_docs, dmap), 2))
                actual_mattina = cash_doc.get("mattina", "")
                manual_mattina = (
                    cash_doc.get("mattina_auto_carry") is False
                    or (date_str, "cash", "mattina") in manual_audits
                )
                if expected_mattina and not manual_mattina and not _report_numbers_equal(actual_mattina, expected_mattina):
                    _data_integrity_issue(
                        issues,
                        severity="critical",
                        area="Report",
                        kind="cash_mattina_mismatch",
                        title="Cash mattina non allineato",
                        message=f"{label}: cash mattina del {date_str} diverso dal cash sera del {prev_date}.",
                        restaurant_id=rid,
                        restaurant_label=label,
                        date=date_str,
                        expected=expected_mattina,
                        actual=actual_mattina,
                        details={"previous_date": prev_date},
                    )

                for cf in CASSETTO_FIELDS:
                    sp_field = CASSETTO_SPICCI_FIELD[cf]
                    prev_cd = str(prev_cash.get(cf) or "").strip()
                    prev_sp = str(prev_cash.get(sp_field) or "").strip()
                    if not prev_cd and not prev_sp:
                        continue
                    checks["cassetto_spicci"] += 1
                    expected_cassetto = _format_report_number(_eval_cash_value(prev_cd) - _eval_cash_value(prev_sp))
                    actual_cassetto = cash_doc.get(cf, "")
                    manual_cassetto = (
                        cash_doc.get(f"{cf}_auto_carry") is False
                        or (date_str, "cash", cf) in manual_audits
                    )
                    if expected_cassetto and not manual_cassetto and not _report_numbers_equal(actual_cassetto, expected_cassetto):
                        _data_integrity_issue(
                            issues,
                            severity="critical",
                            area="Report",
                            kind="cassetto_spicci_mismatch",
                            title="Cassetto spicci non riportato",
                            message=f"{label}: {cf} del {date_str} non coincide con {cf}-{sp_field} del {prev_date}.",
                            restaurant_id=rid,
                            restaurant_label=label,
                            date=date_str,
                            expected=expected_cassetto,
                            actual=actual_cassetto,
                            details={"field": cf, "spicci_field": sp_field, "previous_date": prev_date},
                        )

            if date_str < today and not cash_doc.get("paste_manual_override"):
                saved_paste = str(cash_doc.get("paste_text") or "").strip()
                if not saved_paste:
                    checks["paste_snapshot"] += 1
                    start_utc, end_utc = _rome_date_bounds_utc(date_str)
                    ...
                    total_orders = active_count + archived_count
                    if total_orders > 0:
                        _data_integrity_issue(
                            issues,
                            severity="warning",
                            area="Report",
                            kind="paste_snapshot_mancante",
                            title="Paste mancanti nel report",
                            message=f"{label}: {total_orders} ordini del {date_str}, ma il campo paste del report e vuoto.",
                            restaurant_id=rid,
                            restaurant_label=label,
                            date=date_str,
                            expected=f"{total_orders} ordini",
                            actual="paste vuote",
                        )
        ...
        beverage_docs = await db.beverage_daily_counts.find(
            {"restaurant_id": rid, "date_rome": {"$gte": cutoff, "$lte": today}},
            {"_id": 0},
        ).sort([("date_rome", 1), ("sigla", 1)]).to_list((days + 10) * max(1, len(valid_beverage_siglas)))
        for bev_doc in beverage_docs:
            sigla = bev_doc.get("sigla") or ""
            date_str = bev_doc.get("date_rome") or ""
            if not sigla or sigla not in valid_beverage_siglas or not date_str:
                continue
            prev_bev = await db.beverage_daily_counts.find_one(
                {"restaurant_id": rid, "sigla": sigla, "date_rome": {"$lt": date_str}},
                sort=[("date_rome", -1)],
                projection={"_id": 0, "date_rome": 1, "sera": 1},
            )
            if not prev_bev:
                continue
            expected_bev = str(prev_bev.get("sera") or "").strip()
            if not expected_bev:
                continue
            checks["bevande_mattina"] += 1
            actual_bev = bev_doc.get("mattina", "")
            manual_bev = (
                bev_doc.get("mattina_auto_carry") is False
                or (date_str, "beverage", f"{sigla}.mattina") in manual_audits
            )
            if not manual_bev and not _report_numbers_equal(actual_bev, expected_bev):
                _data_integrity_issue(
                    issues,
                    severity="critical",
                    area="Report",
                    kind="bevanda_mattina_mismatch",
                    title="Bevanda mattina non allineata",
                    message=f"{label}: {sigla} mattina del {date_str} diverso da sera del {prev_bev.get('date_rome')}.",
                    restaurant_id=rid,
                    restaurant_label=label,
                    date=date_str,
                    expected=expected_bev,
                    actual=actual_bev,
                    details={"sigla": sigla, "previous_date": prev_bev.get("date_rome") or ""},
                )

        checks["contatori_ordini"] += 1
        highest_today = await _highest_order_number_today(rid)
        current_counter = _safe_int(rest.get("order_counter"), 0)
        if current_counter < highest_today:
            _data_integrity_issue(
                issues,
                severity="critical",
                area="Ordini",
                kind="order_counter_basso",
                title="Contatore ordini troppo basso",
                message=f"{label}: il prossimo numero rischia di sovrapporsi agli ordini di oggi.",
                restaurant_id=rid,
                restaurant_label=label,
                date=today,
                expected=f">= {highest_today}",
                actual=current_counter,
            )

    today_start_utc, _ = _today_rome_bounds_utc()
    stale_orders = await db.orders.find(
        {"created_at": {"$lt": today_start_utc}},
        {"_id": 0, "restaurant_id": 1, "order_number": 1, "created_at": 1},
    ).sort("created_at", 1).to_list(500)
    if stale_orders:
        grouped: Dict[str, List[Dict]] = {}
        for order in stale_orders:
            grouped.setdefault(order.get("restaurant_id") or "", []).append(order)
        for rid, rows in grouped.items():
            checks["ordini_vecchi"] += len(rows)
            sample_numbers = [str(o.get("order_number")) for o in rows[:6]]
            label = rest_map.get(rid, rid or "Sconosciuto")
            _data_integrity_issue(
                issues,
                severity="critical",
                area="Ordini",
                kind="ordini_attivi_vecchi",
                title="Ordini attivi di giorni precedenti",
                message=f"{label}: {len(rows)} ordini ancora attivi prima di oggi.",
                restaurant_id=rid,
                restaurant_label=label,
                date=_rome_date_from_iso(rows[0].get("created_at", "")) or "",
                expected="archiviati",
                actual=", ".join(sample_numbers),
                details={"count": len(rows), "oldest_created_at": rows[0].get("created_at", "")},
            )

    duplicate_orders = await db.orders.aggregate([
        {"$group": {
            "_id": {"restaurant_id": "$restaurant_id", "order_number": "$order_number"},
            "count": {"$sum": 1},
            "ids": {"$push": "$id"},
        }},
        {"$match": {"count": {"$gt": 1}}},
    ...
    for dup in duplicate_orders:
        checks["ordini_duplicati"] += 1
        rid = (dup.get("_id") or {}).get("restaurant_id") or ""
        order_number = (dup.get("_id") or {}).get("order_number")
        label = rest_map.get(rid, rid or "Sconosciuto")
        _data_integrity_issue(
            issues,
            severity="critical",
            area="Ordini",
            kind="ordine_numero_duplicato",
            title="Numero ordine duplicato",
            message=f"{label}: numero ordine {order_number} presente {dup.get('count')} volte tra gli ordini attivi.",
            restaurant_id=rid,
            restaurant_label=label,
            date=today,
            expected="unico",
            actual=dup.get("count"),
            details={"order_number": order_number, "ids": dup.get("ids", [])[:10]},
        )

    products = await db.products.find(
        {},
        {"_id": 0, "id": 1, "name": 1, "quantity": 1, "supplier": 1},
    ).sort("name", 1).to_list(2000)
    for product in products:
        product_id = product.get("id") or ""
        product_name = product.get("name") or product_id or "Prodotto"
        qty = _safe_int(product.get("quantity"), 0)
        if qty < 0:
            checks["stock_negativo"] += 1
            _data_integrity_issue(
                issues,
                severity="critical",
                area="Magazzino",
                ...
                message=f"{product_name}: quantita prodotto negativa.",
                expected=">= 0",
                actual=qty,
                details={"product_id": product_id, "supplier": product.get("supplier") or ""},
            )
        if product_id:
            last_movement = await db.stock_movements.find_one(
                {"product_id": product_id},
                sort=[("timestamp", -1)],
                projection={"_id": 0, "balance_after": 1, "timestamp": 1, "cause": 1},
            )
            if last_movement and _safe_int(last_movement.get("balance_after"), qty) != qty:
                checks["ledger_stock"] += 1
                _data_integrity_issue(
                    issues,
                    severity="warning",
                    area="Magazzino",
                    kind="ledger_stock_mismatch",
                    title="Ledger stock non allineato",
                    message=f"{product_name}: la quantita prodotto non coincide con l'ultimo movimento stock.",
                    expected=last_movement.get("balance_after"),
                    actual=qty,
                    details={
                        "product_id": product_id,
                        "last_movement_at": last_movement.get("timestamp") or "",
                        "last_movement_cause": last_movement.get("cause") or "",
                    },
                )

    upload_checks = [
        ("invoices", "Fatture", ["image_file"]),
        ("versamenti", "Versamenti", ["image_file"]),
        ("chiusure", "Chiusure", ["image_file", "piatti_file"]),
        ("carichi_magazzino", "Carichi magazzino", ["photo_file", "fattura_file"]),
        ("beverage_carichi", "Carichi bevande", ["invoice_file"]),
        ("fatture_globali", "Fatture globali", ["image_file"]),
    ]
    for coll_name, area_label, fields in upload_checks:
        projection = {"_id": 0, "id": 1, "restaurant_id": 1, "created_at": 1}
        for f in fields:
            projection[f] = 1
        docs = await db[coll_name].find({}, projection).sort("created_at", -1).to_list(1000)
        for doc in docs:
            rid = doc.get("restaurant_id") or ""
            label = rest_map.get(rid, "")
            for field in fields:
                filename = (doc.get(field) or "").strip() if isinstance(doc.get(field), str) else ""
                if not filename:
                    continue
                if not (UPLOADS_DIR / filename).exists():
                    checks["file_upload"] += 1
                    _data_integrity_issue(
                        issues,
                        severity="warning",
                        area=area_label,
                        kind="upload_file_mancante",
                        title="File upload mancante",
                        message=f"{area_label}: file {filename} non trovato in uploads.",
                        restaurant_id=rid,
                        restaurant_label=label,
                        date=_rome_date_from_iso(doc.get("created_at", "")) or "",
                        expected="file presente",
                        actual=filename,
                        details={"collection": coll_name, "document_id": doc.get("id") or "", "field": field},
                    )
    ...
    return {
        ...
        "today": today,
        "summary": {
            "ok": counts["critical"] == 0 and counts["warning"] == 0,
